...
<body>
				...
				<li class="nav-item dropdown pe-3"><a
					...
					<ul
						...
						<li class="dropdown-header">
							<h6><%=session.getAttribute("name")%></h6> <span>Admin -<%=session.getAttribute("role")%></span>
						</li>
						<li>
							<hr class="dropdown-divider">
						</li>
	...
	<aside id="sidebar" class="sidebar">
		...
		<ul class="sidebar-nav" id="sidebar-nav">
			...
			<li class="nav-item"><a class="nav-link collapsed"
				href="addtask.jsp"> <i class="bi bi-clipboard2-plus-fill"></i> <span>Add
						Task</span>
			</a></li>
			<li class="nav-item"><a class="nav-link collapsed"
				href="users-profile.jsp"> <i class="bi bi-menu-button-wide"></i> <span>Your Profile</span>
			</a></li>
			
			<li class="nav-item"><a class="nav-link collapsed" href="task.jsp">
	...
	<main id="main" class="main">
		...
		<!-- End Page Title -->
		<%
		String query = "SELECT *,users.id as uid,tasks.id as tid  FROM `tasks`,users WHERE tasks.user_id = users.id";
						...
						<table class="table table-striped datatable">
							...
							<tbody>
								...
								<tr>
									...
									<td><a
										href="updateStatus.jsp?id=<%=rs.getString("tid")%>&status=<%=rs.getString("status")%> "><span
											class="badge bg-<%=statusString%>"><%=rs.getString("status")%>
										</span></a></td>
									<td><a href="deleteTask.jsp?id=<%=rs.getString("tid")%>" type="button" class="btn btn-sm btn-danger">
